...
class FloatingItem:
    ...
    def __init__(self, screen_w: int, screen_h: int, color: tuple[int, int, int] | None = None) -> None:
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        pygame.draw.circle(surf, (*self.color, alpha), (self.size, self.size), self.size // 2)
...
class SteamParticle:
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        if alpha > 0:
            surf = pygame.Surface((self.size * 2, self.size * 2), pygame.SRCALPHA)
            pygame.draw.circle(surf, (255, 255, 255, alpha), (self.size, self.size), self.size)
            screen.blit(surf, (int(self.x) - self.size, int(self.y) - self.size))
